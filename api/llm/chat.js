...
function sendJson(res, statusCode, payload, extraHeaders = {}) {
  ...
  Object.entries(extraHeaders).forEach(([key, value]) => {
    if (value != null && value !== '') res.setHeader(key, String(value))
  })
...
function getProviderConfig(provider) {
  ...
  if (provider === 'openai') {
    ...
    return { provider: 'openai', apiUrl: OPENAI_API_URL, headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${openaiKey}` }, model: DEFAULT_OPENAI_MODEL }
  }
  if (provider === 'anthropic') {
    if (!anthropicKey) return null
    return { provider: 'anthropic', apiUrl: ANTHROPIC_API_URL, headers: { 'Content-Type': 'application/json', 'x-api-key': anthropicKey, 'anthropic-version': '2023-06-01' }, model: DEFAULT_ANTHROPIC_MODEL }
  }
  return null
}
...
export default async function handler(req, res) {
  ...
  if (!providerConfig) {
    sendJson(res, 503, { error: 'missing_api_key', provider }, { 'x-llm-provider': provider, 'x-llm-error-code': 'missing_api_key' })
    return
  }
  ...
  if (!providerBody.model) providerBody.model = providerConfig.model
  try {
    const upstreamResponse = await fetch(providerConfig.apiUrl, { method: 'POST', headers: providerConfig.headers, body: JSON.stringify(providerBody) })
    ...
    if (!upstreamResponse.ok) {
      let parsed = {}
      try { parsed = JSON.parse(bodyText) } catch { parsed = { raw: bodyText.slice(0, 500) } }
      const errorPayload = {
        error: parsed?.error?.message || parsed?.error || 'upstream_error',
        message: parsed?.error?.message || parsed?.message || `${provider} returned ${upstreamResponse.status}`,
        status: upstreamResponse.status,
        provider,
        ...(parsed?.error?.type ? { type: parsed.error.type } : {}),
        ...(parsed?.error?.code ? { code: parsed.error.code } : {}),
      }
      sendJson(res, upstreamResponse.status, errorPayload, {
        'x-llm-provider': provider,
        'x-llm-error-code': String(parsed?.error?.code || parsed?.error?.type || upstreamResponse.status),
    ...
    sendJson(res, 502, { error: 'upstream_request_failed', message: error?.message || 'Unknown error', provider }, { 'x-llm-provider': provider, 'x-llm-error-code': 'upstream_request_failed' })
  }
}
