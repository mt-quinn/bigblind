...
function getVoiceSettings(speaker) {
  if (speaker === 'dater') {
    return { stability: 0.35, similarity_boost: 0.75, style: 0.75, use_speaker_boost: true }
  }
  if (speaker === 'narrator') {
    return { stability: 0.55, similarity_boost: 0.75, style: 0.5, use_speaker_boost: true }
  }
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    sendJson(res, 503, { error: 'missing_api_key' }, { 'x-tts-error-code': 'missing_api_key' })
    return
  }
  ...
  if (text.length > MAX_TEXT_LENGTH) { sendJson(res, 400, { error: `Text too long (max ${MAX_TEXT_LENGTH} chars).` }); return }
  try {
    const upstreamResponse = await fetch(
      `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=mp3_44100_128`,
      {
        ...
        body: JSON.stringify({ text, model_id: 'eleven_multilingual_v2', voice_settings: getVoiceSettings(speaker) }),
      }
    )
    if (!upstreamResponse.ok) {
      const errorText = await upstreamResponse.text()
      sendJson(res, upstreamResponse.status, { error: 'elevenlabs_error', details: errorText.slice(0, 3000) })
      return
    }
    ...
    sendJson(res, 502, { error: 'upstream_request_failed', message: error?.message || 'Unknown error' })
  }
}
