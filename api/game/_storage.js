...
const LOCAL_STORE_FILE = path.join(os.tmpdir(), 'game-template-kv-v1.json')
...
function throwDurableStorageUnavailable(op) {
  ...
  const error = new Error(
    `Game storage unavailable during ${op}. Configure Redis/KV for Vercel (${details}).`
  )
  error.code = 'GAME_STORAGE_UNAVAILABLE'
  throw error
}

function getMemoryStore() {
  const g = globalThis
  if (!g.__GAME_MEM_KV__) {
    g.__GAME_MEM_KV__ = new Map()
  }
  return g.__GAME_MEM_KV__
}
  ...
  try {
    ...
    if (parsedResult) {
      if (parsedResult.recovered) {
        console.warn('Game storage: recovered malformed local store JSON.')
    ...
    console.warn('Game storage: local file read failed, using empty store.', error)
    return {}
  }
}
...
async function writeLocalFileStore(storeObj) {
  ...
  try {
    ...
    console.warn('Game storage: local file write failed, memory fallback.', error)
    return false
  } finally {
    try {
...
async function getVercelKv() {
  ...
  try {
    ...
    console.warn('Game storage: @vercel/kv unavailable, falling back.', error)
    _vercelKv = null
  }
  return _vercelKv
}
...
async function getRedisClient() {
  ...
  try {
    ...
    client.on('error', (error) => {
      console.error('Game Redis error:', error)
    })
    await client.connect()
    _redisClient = client
  } catch (error) {
    console.warn('Game storage: redis unavailable, falling back.', error)
    _redisClient = null
  }
...
export async function kvGetJSON(key) {
  ...
  if (redis) {
    try {
      ...
      console.warn('Game storage: redis read failed, memory fallback.', error)
    }
  }
...
export async function kvSetJSON(key, value, options = {}) {
  ...
  if (redis) {
    try {
      ...
      console.warn('Game storage: redis write failed, memory fallback.', error)
    }
  }
